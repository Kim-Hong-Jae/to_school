...
public class Weather {
	...
	public Weather() throws IOException {
        ...
        if (rainProbability>50) {
        	requirements.add(String.format("우산 %s %", rainProbability));
        }
        if (temperature>25) {
        	requirements.add(String.format("반팔 %s C", temperature));
        }
        else if(temperature>15) {
        	requirements.add(String.format("긴팔 %s C", temperature));
        }
        else if(temperature>5) {
        	requirements.add(String.format("코트 %s C", temperature));
        }
        else {
        	requirements.add(String.format("패딩 %s C", temperature));
        }
    }
	public static ArrayList<String> getRequirements(){
		//TODO 서버로 날씨로 인해 필요 물품 전송
		return requirements;
	}
